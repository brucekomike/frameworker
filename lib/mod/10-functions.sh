...
is_not_reserved() {
  local reserved=("help" "list" "mod" "base" "env" "template")  # Array of reserved words
...
function update_all() {
  ...
  while IFS=" " read -r NAME GIT_URL; do
    ...
    if [[ -d "$LIB_DIR/$NAME" ]]; then
      ...
      (
        cd "$LIB_DIR/$NAME" || script_exit "Failed to navigate to $LIB_DIR/$NAME" 1
        git pull --depth 1 --rebase || script_exit "Failed to update $NAME" 1
      )
    else
      # If the directory does not exist, clone the repository
      verb_echo "Cloning new library: $NAME"
      git clone --depth 1 "$GIT_URL" "$LIB_DIR/$NAME" || script_exit "Failed to clone $NAME from $GIT_URL" 1
    fi
  done < "$CONFIG_FILE"
